...
export const createUser = async (req: AuthRequest, res: Response) => {
    ...
    if (!firebaseUid || !email || !name) {
        return res.status(400).json({ message: "Dados do usuário incompletos para cadastro/sincronização." });
    }

    const db = await dbPromise;
    const checkEmailQuery = "SELECT * FROM usuario WHERE email = $1";
    try {
        const userExistsResult: QueryResult = await db.query(checkEmailQuery, [email]);
        ...
        return res.status(500).json({ message: "Erro ao cadastrar/sincronizar usuário", error: err });
    }
};
...
export const handleGoogleLogin = async (req: AuthRequest, res: Response) => {
    ...
    if (!email || !firebaseUid) {
        return res.status(400).json({ message: "Email e UID do Firebase são obrigatórios." });
    }
    ...
    try {
        ...
        if (userExists.length === 0) {
            console.log("-> handleGoogleLogin: Usuário não existe no DB, criando novo...");
            // Insert sem a senha
            const insertQuery =
                "INSERT INTO usuario (name, email) VALUES ($1, $2) RETURNING id_usuario, name, email";
            const insertResult: QueryResult = await db.query(insertQuery, [
            ...
            console.log("-> handleGoogleLogin: Usuário já existe no DB, retornando dados.");
            return res.status(200).json({
...
export const getUserData = async (req: AuthRequest, res: Response) => {
    ...
    if (!email) {
        return res.status(401).json({ message: "Email do usuário não disponível no token." });
    }

    try {
        const db = await dbPromise;
        // A query não busca 'senha'
        const query = "SELECT id_usuario, name, email, foto_perfil AS profilePhotoUrl, data_criacao AS creationDate FROM usuario WHERE email = $1"; // Ajustei para corresponder ao DB e frontend
